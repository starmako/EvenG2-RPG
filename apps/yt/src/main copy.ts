...
const screenshot = () => {
  ...
  async function captureAndSend() {
    ...
    try {
      ...
      applyG2Green16Quantize(ctx, width, height);
  ...
  canvas.toBlob(
    ...
    //0.5,
      ...
      console.log(new Date().toISOString(), "Captured image size:", bytes.length);
  ...
  captureAndSend();
  intervalId = window.setInterval(captureAndSend, 2500);
};
